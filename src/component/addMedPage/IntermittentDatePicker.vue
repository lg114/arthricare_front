...
<template>
    ...
    @date-update="dateClicked" :disabled="shouldBeDisabled">
...
const getDate = (SelectedDate) => {
    ...
    if (Array.isArray(SelectedDate) && SelectedDate.length > 2) {
        //"Every three days, eat once and repeat this four times."
          if(perferUnit.value == "week")
          {
            const weeks = Math.round(IntervalDays.value / 7);
            formattedOutput.value = `Every ${weeks} weeks eat once and eat ${RepeatTimes.value+2} times `
          }else if(perferUnit.value == "month")
          {
            const months = Math.round(IntervalDays.value / 30);
            formattedOutput.value = `Every ${months} months eat once and eat ${RepeatTimes.value+2} times `
          }else
          {
             formattedOutput.value = `Every ${IntervalDays.value} days eat once and eat ${RepeatTimes.value+2} times `
          }
    }else if(Array.isArray(SelectedDate) && SelectedDate.length == 2)
    {
      formattedOutput.value = `Every ${IntervalDays.value} days eat once and eat 2 times `
    }
