...
const EndDate = ref([new Date(props.StartDateFromParent),new Date(props.EndDateFromParent)]);
...
watch(() => props.EndDateFromParent, (newValue) => {
  ...
  EndDate.value[0] = StartDate.value;
  EndDate.value[1] = new Date(newValue);
}, { deep: true });
...
function checkPeriodType(duraton) {
    if (duraton.includes('day')) {
      return 'day';
    } else if (duraton.includes('week')) {
      return 'week';
    } else if (duraton.includes('month')) {
      return 'month';
    } else {
      return 'year';
    }
